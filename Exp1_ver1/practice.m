...
try
    ...
    %====== Input ======%
        ...
        fName_1 = [subjNo '_Result.txt'];
        fName_2 = [subjNo '_Result_thr.txt'];
    ...
    %====== Experimental Condition ======%
        ...
        % set up condition lists
            ...
            %5 test face
            %6 result
            %7 are there seen faces? 0=no 1=yes
            ...
            for i            =1:5
                temp(70*(i-1)+1: 70*i-35, 4) = repmat(1, 1, repeat*7);
                temp(70*(i-1)+1: 70*i-35, 5) = repmat(2, 1, repeat*7);
                temp(70*i-34: 70*i,4) = repmat(2, 1, repeat*7);
                temp(70*i-34: 70*i,5) = repmat(1, 1, repeat*7);
            end
     ...
     %====== Stimuli Face Threshold ======%
        ...
        for j=1:4 numReportUnseen{j} = 0; end
        ...
        for i= 1:25
             ...
             % Show faces and Mon
                 ...
                 while GetSecs - timezero < 1 & noBreak,  %show faces for 1 sec
                    ...
                    for j = 1:4 %adjust contrast
                        if contrast{j}< upperBound{j}, contrast{j} = contrast{j}+ConIncr;end
                        if contrast{j}> upperBound{j}, contrast{j} = upperBound{j}; end
                    end
                    ...
                    %draw Mondrians
                    for j=1:4
                        Screen('DrawTexture', wPtr, mon.tex{MonIdx}, [], monPosi(j,:));
                    end
            ...
            % Report Faces Visibility
             waitForAnswer = 1;
               ...
               %adjust upperbound
               for j=1:4 thresholdData(i,j) = upperBound{j}; end; %save before adjusting
               
               seeFaces = 0;
               for j = 1:4
                  if(Seen(j))
                     if(upperBound{j} > stepsize & upperBound{j}<1-stepsize) upperBound{j} = upperBound{j}-stepsize; end
                     numReportUnseen{j} = 0; seeFaces = 1;
                  else
                     numReportUnseen{j} = numReportUnseen{j} +1;
                     if numReportUnseen{j} == 2
                         upperBound{j} = upperBound{j} + stepsize;
                         numReportUnseen{j} = 0;
                     end
                  end
               end
